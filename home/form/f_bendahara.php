<?php 
    ...
    {  
        $id = $_GET['id'];
        $cari = mysqli_query($con,"select * from tb_bendahara where id_bendahara='$id' "); 
                                   $tampil = mysqli_fetch_assoc($cari);
        $id= $tampil['id_bendahara'];
        $nama= $tampil['nama'];
        $us= $tampil['username'];
        $ps= $tampil['password'];
        $edit_id= $_GET['id'];
    ...
    {      
        $cari_id = mysqli_query($con,"select coalesce(max(right(id_bendahara,3)),0)+1 as kd from tb_bendahara") or die('gagal pencarian data !');           
...
<label style="margin-left:25px;"><b> <font size="5"> FORM BENDAHARA </font> </b></label></br></br>
...
<tr>
                                        ...
                                        <td style="border:0px solid #999;">    <button type="button" onclick="tampil('tabelbendahara','','','')" >Kembali</button> <button style="margin-left:5px;" type="submit" onclick="val_bendahara('<?php echo $edit_id; ?>');" > Simpan </button> </td>
</tr>
</table>
</form>
