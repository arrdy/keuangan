...
<script>
	...
	$(document).ready(function(){
			$("#rekening").autocomplete({
				minLength:2,
				source:'autoc/cari_rek.php?cari=penyesuaian',
...
<?php 
    ...
    {  
        $id = $_GET['id'];
        $cari = mysqli_query($con,"select t.*,p.id_investor from tb_jp t left join tb_perjanjian p on p.id_notaris=t.id_notaris where t.id_jp='$id' "); 
    ...
    {      
    	$cari_id = mysqli_query($con,"select coalesce(max(right(id_jp,3)),0)+1 as kd from tb_jp") or die('gagal pencarian data !');           
...
<div style="margin-top:-50;">
					<h2 ><center><strong>FORM PENYESUAIAN</strong></center></h2>
...
<table width="90%" cellpadding="2%" style="border:0px;" align="center" >
	...
	<tr>
		...
		<td style="border:0px;"> : </td>
		<td style="border:0px;"> 												<select name="investor">
	...
	<tr>
		<td colspan="3" height="30" valign="bottom" style="border:0px;"><strong> Input Rincian Penyesuaian : </strong></td>
	</tr>
	<tr>
		...
		<td style="border:0px;"><br><br>
			<table width="100%" border="1" style="border-collapse:collapse" id="myTable">
				...
				$n=0;
					...
					if(!empty($_GET['id']))
					{	$edit_id = $_GET['id'];
					$cariju = mysqli_query($con,"select * from tb_d_jp j inner join tb_rekening r on r.no_rekening=j.no_rekening where id_jp='$edit_id' ") or die('gagal pencarian data !'); 
				...
				<tr align="center" bgcolor="skyblue" style="font-weight:bold;">
					...
					<td ></td>
					<input type="text" name="nomor" id="nomor" value="<?php echo $n; ?>" />
				</tr>
				</tfoot>
			</table>
		</td>
	...
	<tr>
		...
		<td style="border:0px;"> 
		<br>
			<input type='button' onclick="val_penyesuaian('<?php echo $edit_id; ?>')" name='simpan' id='simpan' value=' Simpan '/>  
			<input type='Reset' name='reset' value=' Reset ' onClick='deleteAllRows()' /> 
			<input type='button' name='back' value=' Kembali ' onClick="tampil('ttranspenyesuaian','','','')" /> 
		</td>
	</tr>
</table>
</form>	
