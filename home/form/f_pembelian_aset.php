...
<div style="margin-top:-50;">
					<h2 ><center><strong>FORM PEMBELIAN ASET</strong></center></h2>
...
<table width="90%" cellpadding="2%" style="border:0px;" align="center" >
	...
	<tr>
		...
		<br>
			<br>
			<label style="margin-right:10; color:teal; font-size:14; margin-left:12;"> <b> Menggunakan uang dari </b> </label>:
            ...
            <br>    
                    <div style="margin-left:208;"><h1><font color="green"><label id='vsisa'> 0 </label></font></h1>
					<input type="hidden" name="sisa_kas" id="sisa_kas" value="0" /></div>
			 <label style="margin-right:23; color:teal; font-size:14; margin-left:10;"> <b> Nama Aset yang Dibeli  </b></label>:
			 		<select name="rekening1" id="rekening1" >
			 		<option value=""> - Pilih - </option>
                     <?php
                        require_once "koneksi.php"; 
                        $cariagt = mysqli_query($con,"select * from tb_rekening r where left(r.no_rekening,2)='11' "); 
                        if(mysqli_num_rows($cariagt)>0)
                        {
                            while($u = mysqli_fetch_assoc($cariagt))
                            { 
	...
	<tr>
		...
		<td style="border:0px;"> 
		<br>
			<input type='button' onclick="val_pembelian_aset('<?php echo $edit_id; ?>')" name='simpan' id='simpan' value=' Simpan '/>  
			<input type='Reset' name='reset' value=' Reset ' onClick='deleteAllRows()' /> 
			<input type='button' name='back' value=' Kembali ' onClick="tampil('ttranspembelianaset','','','')" /> 
		</td>
	</tr>
</table>
</form>	
