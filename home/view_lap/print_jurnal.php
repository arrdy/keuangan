...
<div style='margin-left:15;'>  
	<strong> <h2> LAPORAN JURNAL </h2>  </strong> 
...
<table border=0 width=98% align=center>
                                    <thead>
                                        <tr class=head bgcolor='darkkhaki'>
                                        	<th width=1% style=text-align : center; height=40>No</th>
                                            <th width=2% style=text-align : center;>Dari</th>
											<th width=3% style=text-align : center;>Tgl transaksi</th>
                                            <th width=40% style=text-align : center;>Ket. Trans</th>
                                            <th width=4% style=text-align : center;>No Rek</th>
...
where tgl_trans between '$tgl1' and '$tgl2' and id_notaris like '%$not%'
union
...
where tgl between '$tgl1' and '$tgl2' and id_notaris like '%$not%'
") or die('gagal pencarian data !'); 
	...
	if($_POST['format']=='1') {
		...
		header("Content-Disposition: attachment; filename=lap_jurnal_$tglnow.doc");
	...
	elseif($_POST['format']=='2') {
		header("Content-type: application/x-msdownload");
		header("Content-Disposition: attachment; filename=lap_jurnal_$tglnow.xls");
		header("Pragma: no-cache");
		header("Expires: 0");
		echo $content;
	}
